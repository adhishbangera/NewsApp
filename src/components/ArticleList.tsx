...
const ArticleList: React.FC<ArticleListProps> = ({ articles,savedPreferences }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 pb-4">
      {articles
      .filter((article) => {
        return !savedPreferences.author || 
          (article.author && article.author.toLowerCase().includes(savedPreferences.author.toLowerCase()))
      })
      .map((article, index) => (
